  ...
  return (
    <nav
      className={`fixed bottom-0 left-0 right-0 z-40 border-t pb-safe ${
        dark
          ? 'bg-[#0c1a2e]/95 border-white/[0.07] backdrop-blur-md'
          : 'bg-white/95 border-stone-200 backdrop-blur-md shadow-lg'
      }`}
    >
        ...
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <span
                  className={`absolute top-0 left-1/2 h-0.5 w-8 -translate-x-1/2 rounded-full ${dark ? 'bg-amber-400' : 'bg-amber-600'}`}
                />
              )}
            </Link>
          );
