...
export default function ExportReport({ dark, duas, onClose }: ExportReportProps) {
  ...
  const generateReport = () => {
    ...
    setTimeout(() => {
      ...
      const allData = load<Record<string, Record<string, boolean>>>(STORAGE_KEY, {});
      const streak = load<Streak>(STREAK_KEY, { current: 0, best: 0, lastComplete: '' });
      const today = getTodayKey();
      const total = duas.length;

      // Build day-by-day data
      const dayRows: { key: string; label: string; cnt: number; pct: number }[] = [];
      ...
      const duaStats = duas.map((d) => {
        ...
        return { ...d, completedDays, rate: Math.round((completedDays / days) * 100) };
      });
  ...
  return (
    ...
    >
      <div className={`w-full max-w-sm rounded-2xl border p-6 shadow-2xl ${panel}`}>
        ...
        <p className={`mt-3 text-[10px] ${dark ? 'text-stone-700' : 'text-stone-400'}`}>
