      ...
      >
        ...
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p
              className={`mt-0.5 text-[10px] ${dark ? 'text-stone-600' : 'text-stone-400'}`}
            >
              {isSubscribed
          ...
          <button
            onClick={onClose}
            className={`rounded-full p-1.5 ${dark ? 'text-stone-500 hover:text-stone-300' : 'text-stone-400 hover:text-stone-600'}`}
          >
            ✕
          </button>
        </div>
        ...
        <div className="mb-4">
          <div className="mb-2 flex items-center justify-between">
            <div>
              <p
                className={`text-[13px] font-medium ${dark ? 'text-stone-200' : 'text-stone-700'}`}
              >
                🌅 Morning Reminder
              </p>
              <p
                className={`text-[10px] ${dark ? 'text-stone-600' : 'text-stone-400'}`}
              >
                Morning adhkār & du`ā
              </p>
            </div>
        ...
        {status === 'error' && (
          <div
            className={`mb-4 rounded-xl border px-3 py-2 text-[12px] ${dark ? 'border-red-500/30 bg-red-500/10 text-red-400' : 'border-red-200 bg-red-50 text-red-600'}`}
          >
            {errorMsg}
          </div>
        )}
        ...
        {/* Info */}
        <p
          className={`mb-4 text-[10px] leading-relaxed ${dark ? 'text-stone-600' : 'text-stone-400'}`}
        >
          Reminders are sent by the server at your scheduled time, so they
          arrive even when the app is closed. Your timezone is detected
          automatically.
        </p>
